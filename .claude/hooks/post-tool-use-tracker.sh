...
parse_json_value() {
    ...
    # Handles both "key": "value" and nested "key": { "nested": "value" }
...
# Extract relevant data - try jq first, fall back to bash parsing
if command -v jq >/dev/null 2>&1; then
    # jq is available - use it for robust parsing
    tool_name=$(echo "$tool_info" | jq -r '.tool_name // empty')
    file_path=$(echo "$tool_info" | jq -r '.tool_input.file_path // empty')
    session_id=$(echo "$tool_info" | jq -r '.session_id // empty')
else
    # jq not available - use bash fallback
